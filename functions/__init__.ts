import { CloudVirtualMachineZone, VirtualMachinePackageList, Region } from './entity'
...
async function createPrincePackageList() {
  ...
  const existed = await db.collection<VirtualMachinePackageList>('PrincePackageList').countDocuments()

  if (existed) {
    return
  }
  await db.collection<VirtualMachinePackageList>('PrincePackageList').insertMany(princePackageList)
}
